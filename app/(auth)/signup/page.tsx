    ...
    <form onSubmit={onSubmit} className="space-y-4">
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div className="space-y-2">
          <Label htmlFor="firstName" className="dark:text-slate-200">
            {signupTranslations[language].firstNameLabel}
          </Label>
          <Input
            id="firstName"
            name="firstName"
        ...
        <div className="space-y-2">
          <Label htmlFor="lastName" className="dark:text-slate-200">
            {signupTranslations[language].lastNameLabel}
          </Label>
          <Input
            id="lastName"
            name="lastName"
      ...
      <div className="space-y-2">
        <Label htmlFor="email" className="dark:text-slate-200">
          {signupTranslations[language].emailLabel}
        </Label>
        <Input
          id="email"
          name="email"
      ...
      <div className="space-y-2 relative w-full">
        <Label htmlFor="password" className="dark:text-slate-200">
          {signupTranslations[language].passwordLabel}
        </Label>
...
export default function SignUpPage() {
  ...
  async function onSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      alert(
        "Please verify your email! You will be automatically logged in after clicking the link.",
      );
    } catch (error) {
      setError(
        error instanceof Error
          ? error.message
          : signupTranslations[language].unexpectedError,
      );
    } finally {
      setIsLoading(false);
    }
            ...
            <Card className="border-purple-100 shadow-md dark:border-purple-800 dark:bg-slate-800">
              <CardHeader className="space-y-1">
                <CardTitle className="text-2xl font-bold text-center">
                  {signupTranslations[language].title}
                </CardTitle>
