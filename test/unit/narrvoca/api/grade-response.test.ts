...
function setupOpenAI(score: number, feedback: string) {
  mockCreate.mockResolvedValue({
    choices: [
      {
        message: {
          content: JSON.stringify({ accuracy_score: score, feedback }),
        },
      },
    ],
  });
...
beforeEach(() => {
  ...
  mockSUFU.mockReturnValue({ from: () => ({ select: mockSelect }) });
});
...
describe("POST /api/narrvoca/grade-response", () => {
  ...
  it("returns 401 when no authorization token is provided", async () => {
    const req = makeReq(
      "POST",
      { node_id: 1, user_input: "Hola", target_language: "es" },
      false,
    );
    const res = makeRes();
    await handler(req as NextApiRequest, res as NextApiResponse);
