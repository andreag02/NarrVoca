...
beforeEach(() => {
  ...
  global.fetch = jest.fn().mockResolvedValue({
    ok: true,
    json: () =>
      Promise.resolve({ accuracy_score: 0.8, feedback: "Good work!" }),
  });
});
...
async function mountWithStory() {
  ...
  await act(async () => {
    await result.current.selectStory(1);
  });
  return result;
}
...
async function mountAtCheckpoint() {
  ...
  act(() => {
    result.current.setUserInput("Hola mundo");
  });
  return result;
}
...
describe("initialisation", () => {
  it("redirects to /login when there is no session", async () => {
    ...
    // Story list must never be populated when there is no session
...
describe("handleSubmit", () => {
  ...
  it("calls grade-response API before logging the interaction", async () => {
    ...
    const gradeIdx = calls.findIndex(
      (url: unknown) => url === "/api/narrvoca/grade-response",
    );
    const logIdx = calls.findIndex(
      (url: unknown) => url === "/api/narrvoca/log-interaction",
    );
  ...
  it("calls update-mastery for each vocab word associated with the node", async () => {
    ...
    const masteryCallUrls = (global.fetch as jest.Mock).mock.calls.filter(
      (c: unknown[]) => c[0] === "/api/narrvoca/update-mastery",
    );
...
describe("resetStory", () => {
  it("clears fullStory, resets nodeIndex, clears userInput, clears isComplete", async () => {
    const result = await mountWithStory();
    act(() => {
      result.current.setUserInput("some text");
    });
    act(() => {
